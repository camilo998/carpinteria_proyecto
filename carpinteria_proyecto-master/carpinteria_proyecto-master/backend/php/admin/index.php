    ...
    if (empty($nombre) || $precio <= 0) {
        ...
        try {
            if ($producto_id > 0) {
                ...
                $stmt->execute([$nombre, $descripcion, $precio, $stock, $imagen, $categoria_id, $producto_id]);
                $success = 'Producto actualizado exitosamente';
            } else {
                // Crear producto
                $stmt = $pdo->prepare("INSERT INTO productos (nombre, descripcion, precio, stock, imagen, categoria_id, activo, destacado) VALUES (?, ?, ?, ?, ?, ?, 1, 0)");
...
<header>
    ...
    <div class="container">
        <div class="admin-section">
            ...
            <p>Desde aquí puedes gestionar los productos de la carpintería.</p>
            ...
            <form method="POST" action="">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" class="form-control" id="stock" name="stock" min="0" value="<?php echo htmlspecialchars($producto_editar['stock'] ?? 0); ?>">
                    </div>
                    <div class="col-md-3 mb-3">
                                ...
                                <?php foreach ($productos as $prod): ?>
                                    <tr>
                                        <td>
                                            ...
                                            $ruta_base = '../../../frontend/views/Carpintin-Don-Gusto/'; // ruta relativa desde localhost
                                            ...
                                            if (!empty($imagen)) {
                                                if (str_starts_with($imagen, 'http')) {
                                                    ...
                                                    // Si es archivo local, agregar ruta relativa
                                                    $ruta_img = $ruta_base;
                                                }
                                            ...
                                            <img src="<?php echo $imagen ? htmlspecialchars($imagen) :'/frontend/views/Carpintin-Don-Gusto/img/logo.jpg'; ?>" 
                                                alt="<?php echo htmlspecialchars($prod['nombre']); ?>">
                                        </td>
                                        <td class="nombre"><?php echo htmlspecialchars($prod['nombre']); ?></td>
                                        <td class="descripcion" title="<?php echo htmlspecialchars($prod['descripcion'] ?? ''); ?>">
                                            <?php echo htmlspecialchars($prod['descripcion'] ?? 'Sin descripción'); ?>
                                        </td>
                                        <td class="precio">$<?php echo number_format($prod['precio'], 2); ?></td>
                                        <td><?php echo htmlspecialchars($prod['stock'] ?? 0); ?></td>
                                        ...
                                        <td class="actions">
                                            ...
                                            <a href="?eliminar=<?php echo $prod['id']; ?>" class="btn btn-delete" 
